#!/usr/bin/env python3
"""
plan_005_fixed: ASV Framework Extension (C1/C2/W1/W2 fixes)

Fixes applied (Reviewer CRITICAL):
  C1: Added compute_signif_asv_fast from plan_004_v3_fixed
  C2: compute_magnitude_asv_fast returns 0.0 (not None) when tau_true ~ 0
  W1: IV tau_true = plim_iv(I), not DGP beta (approx 0.66 vs 0.5)
  W2: M-bias tau_true = plim_mbias(I)[1], not DGP tau (approx 0.5128 vs 0.5)

Only Part 2+3+5 re-run. Part 1 and Part 4 preserved but skipped.
Part 5 uses significance-based ASV rejection rule (from plan_004_v3_fixed).
"""
...
def compute_magnitude_asv_fast(tau_curve, deltas, tau_true, rel_thresh=0.1):
    # [C2] tau~0 means maximally fragile — any annotation error changes the result
    if abs(tau_true) < 1e-12:
        return 0.0
...
# [C1] Significance-based ASV from plan_004_v3_fixed
def compute_signif_asv_fast(tau_curve, deltas, se):
    """Min delta where |plim tau(C(delta))| < 1.96 * SE (becomes non-significant)."""
    z_crit = 1.96
    for i in range(len(deltas)):
        if np.isnan(tau_curve[i]):
            continue
        if abs(tau_curve[i]) < z_crit * se:
            return deltas[i]
    return None


# ============================================================================
# Part 1: Envelope Monotonicity (preserved, skipped in main)
# ============================================================================
...
# ============================================================================
# Part 2: ASV Values (tau=0.5) — [W1/W2] fixed tau_true
# ============================================================================

def part2_asv_values(st_mb, st_iv, st_fd, tau_I_mb, tau_I_iv, tau_I_fd):
    print("\n" + "=" * 70)
    ...
    # [W1/W2] Use plim(I) as tau_true — ASV measures ADDITIONAL bias from annotation error
    for topo, st, tau_true, label in [
        ("mbias", st_mb, tau_I_mb, "M-bias"),
        ("iv", st_iv, tau_I_iv, "IV"),
        ("frontdoor", st_fd, tau_I_fd, "Front-door"),
    ]:
        for cname, C0 in C_list.items():
...
# ============================================================================
# Part 3: Small-tau ASV — [W1/W2] fixed tau_true
# ============================================================================
...
def part3_small_tau_asv():
    ...
    for tau_val in [0.05, 0.1]:
        ...
        # [W1/W2] Use plim(I) as tau_true, not DGP parameter
        tau_I_mb = plim_mbias(np.eye(K), st_mb)[1]
        tau_I_iv = plim_iv(np.eye(K), st_iv)
        print(f"  M-bias tau_I = {tau_I_mb:.6f}, IV tau_I = {tau_I_iv:.6f}")
...
# ============================================================================
# Part 4: MC Coverage (preserved, skipped in main)
# ============================================================================
...
# ============================================================================
# Part 5: Type I/II (Significance-Based ASV Filter) — REWRITTEN
# ============================================================================

def part5_type_errors(n_mc=500, N_sample=5000):
    print("\n" + "=" * 70)
    print(f"PART 5: Type I/II ({n_mc} MC x N={N_sample})")
    print("  Filter: significance-based ASV")
    print("  reject if |tau_hat/SE|>1.96 AND (signif_ASV is None OR signif_ASV > 1)")
    print("=" * 70)
    t0 = time.time()
    param = LinearInterpolation()
    C_list = normalize_C_list(make_C_list())
    ...
    for tau_val in [0.05, 0.1]:
        ...
        header = (f"  {'C_struct':<12} {'H':<5} {'naive_rej':>10} "
                  f"{'asv_rej':>10} {'mean_sasv':>10} {'mag_asv':>10}")
        ...
        for h_label, tau_h in [("H0", 0.0), ("H1", tau_val)]:
            ...
            # [W2] Use plim(I) as tau_true for magnitude ASV
            tau_I_h = plim_mbias(np.eye(K), st_h)[1]
            for cname, C0 in C_list.items():
                seed_ctr += 1
                dmin, dmax = param.delta_range(C0)
                ...
                masv = compute_magnitude_asv_fast(
                    tau_curve, deltas, tau_I_h, 0.1)
                rng_mc = np.random.default_rng(seed_ctr)
                naive_rej = 0; asv_rej = 0; sasv_vals = []
                for mc in range(n_mc):
                    ...
                    if reject:
                        naive_rej += 1
                    sasv = compute_signif_asv_fast(tau_curve, deltas, se_tau)
                    sasv_vals.append(sasv)
                    if reject and (sasv is None or sasv > 1.0):
                        asv_rej += 1
                sf = [a for a in sasv_vals if a is not None]
                sm = f"{np.mean(sf):.2f}" if sf else "N/A"
                ms = f"{masv:.2f}" if masv is not None else "None"
                print(f"  {cname:<12} {h_label:<5} {naive_rej/n_mc:>10.3f} "
                      f"{asv_rej/n_mc:>10.3f} {sm:>10} {ms:>10}")
                results[("mbias", tau_val, cname, h_label)] = {
                    "naive_rej": naive_rej/n_mc, "asv_rej": asv_rej/n_mc,
                    "mean_sasv": np.mean(sf) if sf else None, "masv": masv}
        ...
        for h_label, tau_h in [("H0", 0.0), ("H1", tau_val)]:
            ...
            # [W1] Use plim(I) as tau_true for magnitude ASV
            tau_I_h = plim_iv(np.eye(K), st_h)
            for cname, C0 in C_list.items():
                seed_ctr += 1
                dmin, dmax = param.delta_range(C0)
                deltas = np.linspace(dmin, dmax, 1500)
                ...
                for mc in range(n_mc):
                    ...
                    if reject:
                        naive_rej += 1
                    se_for_asv = se_w if (not np.isnan(se_w) and se_w > 1e-10) else 1e10
                    sasv = compute_signif_asv_fast(tau_curve, deltas, se_for_asv)
                    sasv_vals.append(sasv)
                    if reject and (sasv is None or sasv > 1.0):
                        asv_rej += 1
                sf = [a for a in sasv_vals if a is not None]
...
def collect_summary(asv_res, small_tau_res, cov_res, te_res):
    ...
    summary = {
        "experiment": "plan_005_asv_extended_fixed",
...
if __name__ == "__main__":
    ...
    print("plan_005_fixed: ASV Framework Extension (C1/C2/W1/W2 fixes)")
    print("=" * 70)
    t_total = time.time()
    ...
    # Part 1 skipped (passed review)
    # part1_envelope_monotonicity(st_mb, st_iv, st_fd, tau_I_fd)

    # [W1/W2] Part 2 now uses plim(I) as tau_true
    asv_res = part2_asv_values(st_mb, st_iv, st_fd, tau_I_mb, tau_I_iv, tau_I_fd)

    small_tau_res = part3_small_tau_asv()

    # Part 4 skipped (passed review)
    cov_res = {}
